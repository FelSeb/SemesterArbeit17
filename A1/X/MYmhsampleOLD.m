function [smpl,accept] = MYmhsampleOLD(start,nsamples,varargin)
...
if(returninfo)
    ...
    % output any additional information. The former is a conventional
    % array with each row corresponding to a different chain. The latter is
    % a cell array with every cell row corresponding to a different chain.
    
    % Put the replicates dimension second.
    distnDims = size(start,1);
    ...
    for i = 1-burnin:nsamples*thin
        ...
        y = proprnd(x0, infox0); % sample from proposal dist'n
        
        [logpdfy, infoy] = logpdf(y);
        
        if ~sym
            q1 = logproppdf(x0,y, infoy);
            q2 = logproppdf(y,x0, infox0);
        ...
        if i>0 && mod(i,thin)==0; % burnin and thin
            ...
            for c = 1:nchain
                if(acc(c))
                    smpl.info{i/thin,c} = infoy{c,1};
                    smpl.info2{i/thin,c} = infoy{c,2};
                else
                    smpl.info{i/thin,c} = infox0{c,1};
                    smpl.info2{i/thin,c} = infox0{c,2};
                end
            end
        end;
...
else % (Matlabs built in version of the algorithm)
    % Put the replicates dimension second.
    distnDims = size(start,1);
